...
def split_df_train_test(df, test_size=0.2, random_state=11):
...
def fit_eq1_baseline_train_test(df, group_name="Baseline model",
                                test_size=0.2, random_state=11, initial_guess=None):
...
def fit_eq1_baseline_with_dummy_train_test(df, group_name="Unified model with SAMI dummy",
                                           test_size=0.2, random_state=42, initial_guess=None):
    train_df, test_df = split_df_train_test(df, test_size=test_size, random_state=random_state)
...
def train_model(
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if epoch % verbose_every == 0 or epoch == 1:
            print(f"Epoch {epoch:4d}/{max_epochs} | train_loss={train_loss:.6f} | val_loss={val_loss:.6f}")
...
def build_features_train_test(df, test_size=0.2, random_state=42):
...
def run_direct_model(data_dict, group_name="Type 1", loss_name="huber", max_epochs=300):
    ...
    X_train, X_val, y_train, y_val = train_test_split(
        data_dict["X_train"], data_dict["y_train"], test_size=0.2, random_state=SEED
    )

    train_loader, val_loader = make_loaders(X_train, y_train, X_val, y_val, batch_size=16)
    test_loader = DataLoader(PCIDataset(data_dict["X_test"], data_dict["y_test"]), batch_size=32, shuffle=False)
    ...
    model, history = train_model(
        ...
        loss_name=loss_name, lr=1e-3, max_epochs=max_epochs, patience=40, verbose_every=20
    )
...
def run_transfer_t1_to_t2(data_t1, data_t2, loss_name="huber",
    ...
    X_train_src, X_val_src, y_train_src, y_val_src = train_test_split(
        data_t1["X_train"], data_t1["y_train"], test_size=0.2, random_state=SEED
    )
    src_train_loader, src_val_loader = make_loaders(
        X_train_src, y_train_src, X_val_src, y_val_src, batch_size=16
    )
    ...
    model, src_history = train_model(
        ...
        loss_name=loss_name, lr=1e-3, max_epochs=pretrain_epochs, patience=40, verbose_every=20
    )
    ...
    X_train_tgt, X_val_tgt, y_train_tgt, y_val_tgt = train_test_split(
        X2_train, y2_train, test_size=0.2, random_state=SEED
    )

    tgt_train_loader, tgt_val_loader = make_loaders(
        X_train_tgt, y_train_tgt, X_val_tgt, y_val_tgt, batch_size=8
    )
    tgt_test_loader = DataLoader(PCIDataset(X2_test, y2_test), batch_size=32, shuffle=False)

    print(f"\nFine-tuning on Type 2 with {loss_name.upper()} loss...")
    model, tgt_history = train_model(
        model, tgt_train_loader, tgt_val_loader,
        loss_name=loss_name, lr=5e-4, max_epochs=finetune_epochs, patience=35, verbose_every=20
    )
...
def run_explainable_models(df, group_name="Type 1", test_size=0.2, random_state=42):
    print("\n" + "=" * 100)
...
def main():
    ...
    output_files = [
        ...
        "/content/transfer_t1_to_t2_huber_sensitivity_summary.csv",
    ]

    for folder in ["/content/sensitivity_outputs_dl", "/content/sensitivity_outputs_xai"]:
    ...
    #     files.download("/content/explainable_ai_combined_summary.csv")
